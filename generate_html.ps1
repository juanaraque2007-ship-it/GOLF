...
$html = ""
foreach ($y in $data) {
  $html += "  <div class=`"block-title`" style=`"margin-top: 48px; font-size: 20px;`">Año $($y.year)</div>`n"
  $html += "  <div class=`"doc-list`" style=`"margin-bottom: 24px;`">`n"
  foreach ($doc in $y.docs) {
    $svg = ""
    if ($doc.icon -eq 'presupuesto') {
    ...
    $html += "    <a href=`"$($doc.url)`" target=`"_blank`" class=`"doc-item`">`n"
    $html += "      <div class=`"doc-icon`">`n        $svg`n      </div>`n"
    $html += "      <div class=`"doc-info`">`n"
    $html += "        <div class=`"doc-nombre`">$($doc.name)</div>`n"
    $html += "        <div class=`"doc-tipo`">Documento PDF</div>`n"
    $html += "      </div>`n"
    $html += "      <div class=`"doc-anyo`">$($y.year)</div>`n"
    $html += "    </a>`n"
  }
  $html += "  </div>`n"
}

$orig = Get-Content -Raw -Encoding UTF8 'transparencia.html'
$new = $orig -replace '(?s)<div class="block-title">Documentos Institucionales</div>.*?(?=<div class="block-title">Solicitud de Información</div>)', $html
Set-Content -Path 'transparencia.html' -Value $new -Encoding UTF8
